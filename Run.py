...
def main(args):
    ...
    hyper_source, train_loader, val_loader, test_loader, scaler = get_dataloader(args,
                                                                                 ...
                                                                                 single=False)
    #init model
    adj_path = file_dir+'/FedMetroAGS/data/BJMetro/adjacency.csv'
    ...
    trainer = Trainer(model, loss, optimizer, hyper_source, train_loader, val_loader, test_loader, scaler,
                      args, lr_scheduler=lr_scheduler, logger=args.logger, threshold=threshold)
    ...
    if args.mode == 'train':
        ...
        model.load_state_dict(torch.load('log/BJMetro_in15/embed_6/in6_out6_sp100/best_model.pth'))
        args.logger.info("Load saved model")
        trainer.test(model, trainer.args, hyper_source, test_loader, scaler, trainer.logger)
    else:
        raise ValueError
...
if __name__ == "__main__":
    ...
    args.add_argument('--inter_dropout', default=0.0, type=float)
    ...
    args.log_dir = os.path.join(current_dir, 'log', f"{args.dataset}", f"embed_{args.embed_dim}", f"in{args.lag}_out{args.horizon}_sp{threshold}")
